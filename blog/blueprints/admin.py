...
#视图保护：对于需要登录才能访问的页面，未登录情况下跳转至登录页面
# admin中所有视图都需要视图保护，需要附加login_required装饰器，可以在每个视图函数前添加
# 另一种简单方法是为admin蓝本注册一个before_request处理函数，然后为这个函数附加login_required装饰器，这个函数会在
# 每个请求前运行
